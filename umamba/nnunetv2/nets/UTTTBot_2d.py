...
class TTTLayer(nn.Module):
    def __init__(self, dim, d_state = 16):
        ...
        self.TTTLinear = TTTLinear(
                #  hidden_size=dim, 
                #  num_attention_heads=12, 
                #  num_hidden_layers=d_state, 
                #  mini_batch_size=16, 
                #  share_qk=False, 
                #  conv_kernel=4, 
                #  ttt_base_lr=1,
    ...
    def forward(self, x):
        ...
        # print(f"x_norm size: {x_norm.size()}")
...
class UTTTBot(nn.Module):
    def __init__(self,
        ...
        self.TTT_layer = TTTLayer(dim = features_per_stage[-1])

        self.decoder = UNetResDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision)

    def forward(self, x):
        skips = self.encoder(x)
        skips[-1] = self.TTT_layer(skips[-1])
...
def get_uTTT_bot_2d_from_plans(
    ...
    segmentation_network_class_name = 'UTTTBot'
    network_class = UTTTBot
    kwargs = {
        'UTTTBot': {
            'conv_bias': True,
